...
- Routes each build node to the best-fit specialist from BOTH pools — repo personas (`agents/registry.json`,
  persona-injected) and installed plugin subagents (`node agents/scan-plugins.mjs`, e.g.
  `voltagent-core-dev:backend-developer`, dispatched natively). Highest keyword score wins; build domains
  only (engineering/testing/design/product — see `references/agent-registry.md`); no match → plain
  `general-purpose`. Never routes business-domain agents (scope guard).
...
- `references/agent-registry.md` — **load at Stage 3 dispatch**. Routes build nodes to the best-fit specialist across TWO pools — repo personas (`agents/registry.json`, persona-injected) + installed plugin subagents (`agents/scan-plugins.mjs`, native `subagent_type`); highest-score-wins, build domains only, scope guard.
